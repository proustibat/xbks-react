...
test( 'Should render SearchBar component correctly without a searchTerm content', () => {
    const wrapper = shallow( <SearchBar searchTerm = '' /> );
    expect( wrapper ).toMatchSnapshot();
} );

test( 'Should render SearchBar component correctly with a searchTerm content', () => {
    const wrapper = shallow( <SearchBar searchTerm = 'Hello my searchterm' /> );
    expect( wrapper ).toMatchSnapshot();
} );
...
test( 'Should handle text change', () => {
    ...
    const wrapper = shallow( <SearchBar setSearchTerm={ setSearchTerm } /> );
